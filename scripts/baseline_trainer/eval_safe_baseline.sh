#!/bin/bash
# [Amo] Offline evaluation for one PKU-SafeRLHF baseline experiment:
#   merge LoRA -> generate responses -> amo_eval (per-objective means + HV).
#
# Unlike scripts/amo_exp/eval_pipeline.sh (which used GPU 2,3), this pins ALL
# work to GPU 0,1 and lowers vLLM gpu_memory_utilization so generation coexists
# with the reward gRPC servers already resident on GPU 0,1. amo_eval reuses those
# same servers for scoring. Never touches GPU 2,3.
#
# Writes results/PKU-SafeRLHF/<EXP>.parquet and <EXP>.json (same format as the
# existing qwen2.5-3b_grpo.json etc.).
#
# Usage:
#   bash eval_safe_baseline.sh <EXP> [MODEL]
#     EXP   : experiment name, e.g. qwen2.5-1.5b_ls  (== <MODEL_TAG>_<METHOD>)
#     MODEL : 1.5b | 3b | llama3b  (base model for LoRA merge; inferred from EXP
#             prefix if omitted)
set -x
set -e
...
# Infer base model from the experiment-name prefix if MODEL not given.
if [ -z "$MODEL" ]; then
  case "$EXP" in
...
# 1) merge LoRA adapter -> full HF model (idempotent).
