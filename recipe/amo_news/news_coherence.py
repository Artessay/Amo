"""UniEval-based coherence metric for CNN/DailyMail summarization.
...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info: dict = None) -> float:
    """Compute UniEval coherence score for a single summary.
    ...
    Args:
        ...
        ground_truth: Reference summary (highlights). Unused for coherence.
        extra_info: Per-example metadata. For CNN/DailyMail this must include
            the original `article` string.

    Returns:
        A float coherence score in [0, 1] produced by UniEval.
    """
    del data_source  # Unused but kept for a uniform Amo metric signature.

    return evaluate_summarization(solution_str, ground_truth, extra_info, dim="coherence")
