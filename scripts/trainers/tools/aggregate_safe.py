...
"""[Amo] Aggregate PKU-SafeRLHF trainer results into a comparison table.
...
Usage:
    python scripts/trainers/tools/aggregate_safe.py [--results DIR] [--out FILE]
"""
...
METHOD_LABEL = {
    ...
    "gdpo": "GDPO",
    "hvpo": "HVPO",
    "grpo": "GRPO (Linear Scalarization)",
...
def load_all(results_dir: str) -> dict:
    """Return exp_name -> metrics dict for DATA_SOURCE."""
    out = {}
...
def main():
    ...
    ap.add_argument("--results", default=os.path.join(workspace, "results/PKU-SafeRLHF"))
    ap.add_argument("--out", default=os.path.join(workspace, "results/PKU-SafeRLHF/baselines_table.md"))
    args = ap.parse_args()
    ...
    with open(args.out, "w") as f:
        f.write(text)
    n_done = sum(1 for tag, _ in MODELS for method in METHODS if f"{tag}_{method}" in data)
