...
"""[Amo] Frozen calibration for the PKU-SafeRLHF multi-objective trainers.
...
unlike the MATH objectives which are already in ``[0, 1]``. Several credit
rules are scale-sensitive and therefore need calibration constants that, per
the fair-comparison protocol, must be:
...
Usage (reward servers must be up: ``bash scripts/amo_exp/serve_rewards.sh safe``):
    python scripts/trainers/tools/calibrate_safe.py [--n 512] [--seed 0]
"""
...
# Make ``recipe`` importable when run from any working directory.
...
def main():
    ...
    print(
        f"[calibrate] scoring {len(jobs)} reference responses "
    ...
    # Lagrangian harmlessness budget: median harmlessness of safe references.
    harm = arr[:, 1]
    ...
    calib = {
        ...
        # After affine normalization objectives live in ~[0,1].
        "ideal": [1.0, 1.0],
        # Raw dominated reference for group-level HV weighting (dynamic_hv).
        "hv_reference": [float(lower[0]), float(lower[1])],
        # Raw-scale constraint budget for the Lagrangian baseline.
